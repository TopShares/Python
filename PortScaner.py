...
[Usage]
    ...
    python3 PortScaner.py -u https://www.google.com -p 1-65535

[Value]
    ip              0.0.0.0-255.255.255.255
    top_num         [50|100|1000(default)]
    port_scope      1-65535
    thread_num      [int|23(default)]
        """
        return help
...
def main():
    ...
    if "-h" in args or "--help" in args:
        ...
        sys.exit(1)
    if len(args) >= 5:
        #['PortScaner.py', '-i', 'ip', '-p', '10-100', '-t', 'thread_num']
        ...
        if args[1] == "-i" and args[3] == "-p":
            ...
            if args[4].find("-")!=-1:
                split = args[4].split('-')
                start_port = int(split[0])
                end_port = int(split[1])
            else:
                top = args[4]
            ...
            sys.exit(1)
    elif len(args) == 5: # thread_num default 23
        if args[1] == "-i" and args[3] == "-p":
            ip = args[2]
            if args[4].find("-")!=-1:
                split = port_scner.split()
