...
class Crawler:
    ...
    async def process(self, session, picUrlList):
        ...
        for picUrl in picUrlList:
            ...
            html = await self.getHtmlText(session, picUrl)
            html = etree.HTML(html)
            total = html.xpath('//*[@class="page"]/a[7]/@href')
            total = re.findall('_(\d+).html',total[0])
            total = int(total[0])
            typeNum = re.findall('(\d+).html',picUrl)
            tmp = [URL+typeNum[0]+'_'+str(e)+'.html' for e in range(1,total+1)]
            urlList.append(tmp)
...
def startUrls():
    ...
    rep = re.compile(r'href="/meinv/(.*?).html">末页')
    result = re.findall(rep, html.text)
    total = (((((result[0]).split('/'))[-1:])[0]).split('_'))
    typeNum = total[0]
    total = total[-1:][0]
    urlList = [URL+typeNum+'_'+str(e)+'.html' for e in range(2,int(total)+1)]
    return urlList
